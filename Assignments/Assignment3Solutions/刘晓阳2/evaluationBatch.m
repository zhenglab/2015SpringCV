...
benchPath = 'E:\matlab\assignment3\BSDS500\data\groundTruth\test\';
testPath = 'E:\matlab\assignment3\Label2\0.1\5\40\';
...
%         testImgPath = [testPath testList(i).name];
        testImgPath = strcat(testPath, testList(i).name,'/');
...
%             benchImgName = [benchPath testImgList(j).name];
            benchImgName = strcat(benchPath, testImgList(j).name);
            testImgName = strcat(testImgPath, testImgList(j).name);
...
subplot(224);plot(xFinal,arrayBEFinal,'m'),title('GCE'),xlabel('hr'), ylabel('GCE');
