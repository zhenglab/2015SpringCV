...
%%Setting
InputSeg = '/home/wubin/assignment3/PASCAL_Bin/';
...
for i =1 : length(IdsSeg)
        ...
        if IdsSeg(i).name(1) == '.'     %%检测. ..目录
            ...
            NameSeg = IdsSeg(i).name;       %%列出文件夹下的文件夹名字 4, 5, 6
            PathSeg=strcat(InputSeg, NameSeg, '/');  %%Seg文件夹绝对路径/home/PASCAL_Seg/6/
            ...
            for j =1 : length(IdsSegImg)   %%文件个数，20
                if IdsSegImg(j).name == '.'
                    ...
                    ImgGT=strrep(IdsSegImg(j).name,'-bin.jpg','.png'); %%  img_gt=20.png
